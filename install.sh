...
function install_bot() {
    ...
    ZIP_URL=$(curl -s https://api.github.com/repos/LiamAghamohammadi/botmirzapanel/releases/latest | grep "zipball_url" | cut -d '"' -f 4)

# Check for version flag
if [[ "$1" == "-v" && "$2" == "beta" ]] || [[ "$1" == "-beta" ]] || [[ "$1" == "-" && "$2" == "beta" ]]; then
    ZIP_URL="https://github.com/LiamAghamohammadi/botmirzapanel/archive/refs/heads/main.zip"
elif [[ "$1" == "-v" && -n "$2" ]]; then
    ZIP_URL="https://github.com/LiamAghamohammadi/botmirzapanel/archive/refs/tags/$2.zip"
fi
...
function update_bot() {
    ...
    if [[ "$1" == "-beta" ]] || [[ "$1" == "-v" && "$2" == "beta" ]]; then
        ...
        ZIP_URL=$(curl -s https://api.github.com/repos/LiamAghamohammadi/botmirzapanel/releases/latest | grep "zipball_url" | cut -d '"' -f4)
    fi
...
EOF"
    ...
    git clone https://github.com/LiamAghamohammadi/botmirzapanel.git "$BOT_DIR" || {
...
function update_additional_bot() {
    ...
    if ! git clone https://github.com/LiamAghamohammadi/botmirzapanel.git "$BOT_PATH"; then
